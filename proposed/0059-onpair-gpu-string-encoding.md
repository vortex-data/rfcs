...
(The implementation rollout — a phased landing plan that ships the CPU-only encoding first, then the GPU decoder, then the shared-dictionary layout — is described in §[Implementation Specification](#implementation-specification) below. It is a project-plan decision, not a design decision; the design above is one encoding.)
...
- **GPU decompression is a Vortex priority within the RFC's time horizon.** The CUDA decoder, the `splits` buffer, and the coalesced-format flag are all GPU-driven design choices; if Vortex's roadmap defers GPU decode indefinitely, those features land unused and the RFC's "GPU-friendly" framing is overstated for that timeline. The author's understanding from outside-the-RFC context is that GPU is on the roadmap; readers from the broader Vortex maintainer set should sanity-check this is shared.
...
- **Fixed 12-bit format with no `code_width_bits` parameter.** Simpler. But Vortex deployments differ — CPU-only deployments want the paper-recommended 16-bit; mixed deployments want 12-bit for GPU SMEM fit; GPU-first Ada deployments want 11-bit for occupancy. Single-width forces a global compromise. The parameter is cheap to support (same bitstream shape, different table sizes) and the flexibility is real.
...
- **Encoder code-width selection policy.** The format supports five widths; the encoder needs a default and an override mechanism. Options: (a) caller specifies `code_width_bits` explicitly per array; (b) caller specifies a deployment-target label (`cpu_only` → 16-bit, `mixed` → 12-bit, `gpu_first` → 11-bit) and the encoder picks; (c) encoder picks dynamically from data characteristics (entropy, distinct n-gram count, expected ratio at each width on a sample). Current prior: (b) as the default with (a) as an escape hatch; (c) is appealing but requires the data-characteristics heuristic to be measured before committing. **Falsification:** if validation shows a single width dominates across all tested workloads on H100, drop to a fixed default with (a) as the only override.
...
A project-planning note, not a design choice — included here so implementers and reviewers share a default. The design above is one encoding; it can land in one PR or several. We recommend three independently-mergeable phases:

**Phase 1 — `OnPair16Array` at 16-bit codes, CPU-only.** No `splits` buffer emitted by the encoder, no CUDA decoder shipped, no `OnPair16Layout`. The bytes-on-disk format is the full format spec'd above (the optional `splits` buffer is simply absent; `n_splits = 0`); a Phase-2 reader can read Phase-1 files without modification. Validates that OnPair16 beats FSST on CPU decode and matches FSST on compression speed, exercises the writer+reader plumbing, and lets the Vortex maintainers measure production data on the encoding before the harder pieces ship. The 16-bit width matches the OnPair paper's recommended default for CPU-only deployments.

**Phase 2 — parametric `code_width_bits` + `splits` buffer emission + Mode A CUDA decoder.** Adds the 10/11/12/14-bit widths to the encoder, the optional `splits` buffer, and the GSST one-block-per-SM CUDA decoder. Validates the split-parallel GPU decode and the per-width ratio/throughput tradeoff. Files written in Phase 2 with `code_width_bits != 16` are not readable by Phase 1 readers — this is the first phase that requires reader-version bumps for older files.
