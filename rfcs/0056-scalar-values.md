...
The probe should own its strategy state and must not borrow `ExecutionCtx`. That keeps the probe's
lifetime independent from any one execution context. `ScalarProbe::get` receives `ExecutionCtx` on
every call because probing may perform work lazily: decode a page, read a device buffer, fetch
deferred data, fill a cache, or advance encoding-specific cursor state. Construction chooses the
strategy from the array and options; each read still has the execution context needed to do work.
If an implementation uses context-scoped caches, the probe should store cache keys or owned handles,
not references into the context.
